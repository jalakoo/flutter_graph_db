...
void main() {
  group('IntEqualityRangeIndex', () {
    test('equality + range over an int_ column', () {
      ...
      db.state.nodeProps.setInt(1, age, 30);
      db.state.nodeProps.setInt(2, age, 40);
      db.state.nodeProps.setInt(3, age, 50);
      db.state.nodeProps.setInt(4, age, 50);
    ...
    test('hash overlay returns the matching vids', () {
      ...
      db.state.nodeProps.setInt(0, score, 100);
      db.state.nodeProps.setInt(1, score, 200);
      db.state.nodeProps.setInt(2, score, 100);
    ...
    test('skips tombstoned and explicit-null vids', () {
      final db = _smallDb();
      final age = db.internPropKey('age');
      db.state.nodeProps.setInt(0, age, 10);
      db.state.nodeProps.setInt(1, age, 20);
      db.state.nodeProps.setInt(2, age, 30);
      db.state.nodeProps.setNull(3, age); // explicit null — excluded
  ...
  group('DoubleEqualityRangeIndex', () {
    test('equality + range over a double_ column', () {
      ...
      db.state.nodeProps.setDouble(0, r, 1.5);
      db.state.nodeProps.setDouble(1, r, 2.5);
      db.state.nodeProps.setDouble(2, r, 1.5);
      db.state.nodeProps.setDouble(3, r, 3.0);
  ...
  group('StringIdEqualityRangeIndex', () {
    test('equality + range over a stringId column', () {
      ...
      db.state.nodeProps.setStringId(0, tag, red);
      db.state.nodeProps.setStringId(1, tag, blue);
      db.state.nodeProps.setStringId(2, tag, red);
      db.state.nodeProps.setStringId(3, tag, green);
  ...
  group('StringEqualityRangeIndex', () {
    test('lexicographic equality + range over a string column', () {
      ...
      db.state.nodeProps.setString(0, n, 'Charlie');
      db.state.nodeProps.setString(1, n, 'Alice');
      db.state.nodeProps.setString(2, n, 'Bob');
      db.state.nodeProps.setString(3, n, 'Alice');
  ...
  group('BoolEqualityRangeIndex', () {
    test('cutoff splits false-vids from true-vids', () {
      ...
      db.state.nodeProps.setBool(0, active, true);
      db.state.nodeProps.setBool(1, active, false);
      db.state.nodeProps.setBool(2, active, true);
      db.state.nodeProps.setBool(3, active, false);
      db.state.nodeProps.setBool(4, active, true);
  ...
  group('Registry', () {
    test('rejects duplicate index names', () {
      ...
      db.state.nodeProps.setInt(0, k, 1);
    ...
    test('node and edge registries are independent', () {
      ...
      db.state.nodeProps.setInt(0, k, 1);
      db.state.edgeProps.setInt(0, k, 9);
  ...
  group('Size-event hook', () {
    test('fires when ratio ≥ 25% of CSR', () {
      ...
      final n = db.internPropKey('name');
      for (var v = 0; v < 5; v++) {
        db.state.nodeProps.setString(v, n, 'somewhat_longish_string_$v');
      }
    ...
    test('does not fire for a tiny index well under the threshold', () {
      ...
      final k = db.internPropKey('age');
      db.state.nodeProps.setInt(0, k, 42);
      var fired = false;
